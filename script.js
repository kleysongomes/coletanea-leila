    ...
    fetch('musicas.json')
        ...
        .then(musicas => {
            musicas.forEach((musicaObj) => {
                ...
                if (musicaObj.link) { // Verifica se o link existe no JSON
                    const listenButton = document.createElement('a');
                    listenButton.className = 'listen-button'; // Classe para estilização
                    listenButton.href = musicaObj.link;
                    listenButton.textContent = 'Ouvir agora';
                    listenButton.target = '_blank'; // Abre o link em uma nova aba
                    listenButton.rel = 'noopener noreferrer'; // Boa prática de segurança

                    panel.appendChild(listenButton);
                }
